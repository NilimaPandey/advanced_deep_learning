...
class LoRALinear(HalfLinear):
    def __init__(self, in_features: int, out_features: int, lora_dim: int = 32, bias: bool = True) -> None:
        ...
        self.requires_grad_(False)  # freeze base HalfLinear

        # LoRA adapters (trainable, float32 for stability)
        ...
        # Scale factor (common in LoRA)
        self.scaling = 1.0 / lora_dim

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        base_out = super().forward(x)  # frozen half-precision linear
