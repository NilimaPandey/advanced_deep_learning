...
def block_quantize_4bit(x: torch.Tensor, group_size: int = 128):
    """
    Quantize a 1D tensor into 4-bit blocks with larger group size to reduce memory.
    Returns:
      q4  : [blocks, group_size/2] packed int8
      norm: [blocks, 1] per-block scale factors
    """
    assert x.dim() == 1
    ...
    # pack 2 values (4-bit each) into one int8
...
class LinearGrouped4Bit(torch.nn.Module):
    def __init__(self, in_features: int, out_features: int, bias: bool = True, group_size: int = 128):
        ...
        # Buffers for quantized weights and per-block scales
    ...
    def _load_state_dict_pre_hook(self, state_dict, prefix, *args, **kwargs):
        if f"{prefix}weight" in state_dict:
            ...
            self.weight_q4 = torch.cat(q4_list, dim=0)      # [out, blocks, group_size/2]
            self.weight_norm = torch.cat(norm_list, dim=0)  # [out, blocks, 1]

    def forward(self, x: torch.Tensor):
        # Unpack: [out, blocks, group_size/2] -> [out, blocks, group_size]
        ...
        return torch.nn.functional.linear(x.to(torch.float32), W.detach(), self.bias)
        # note: detach so grads don't flow through quantization math


class BigNetGrouped4Bit(torch.nn.Module):
    class Block(torch.nn.Module):
        def __init__(self, channels, group_size=128):
            super().__init__()
