...
class Linear4Bit(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        q8 = torch.stack((low, high), dim=-1).reshape(weight_q4.size(0), weight_q4.size(1), self._group_size)

        q8 = q8.to(torch.float32) / 15.0
        norms = weight_norm.to(torch.float32).expand(-1, -1, self._group_size)
